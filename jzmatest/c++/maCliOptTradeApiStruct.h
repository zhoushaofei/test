...
struct MATRADEAPI CRspOptUserLoginField
{
  ...
  char          szSubacctCode[8+1];			// 证券账户子编码   2015/8/5
...
struct MATRADEAPI CRspOptBaseInfoField
{
  ...
  char          chCombFlag;			//2015/8/3添加 合约组合标志位
  int           iAutoSplitDate;		//2015/8/3添加 组合策略到期解除日
  int           iUpdDate;                   // 更新日期
...
struct MATRADEAPI CReqOptOrderField
{
  ...
  char          szTrdacct[10 + 1];          // 证券账户
  char          szOptNum[16 + 1];           // 合约编码
  char          szOrderPrice[11 + 1];       // 委托价格 CPRICE4 单位：元，精确到0.1厘
  ...
  char			chEncryptType;	     //2015/8/3添加 加密方式
  char			szEncryptKey[32+1];	//2015/8/3添加  加密因子
};
...
struct MATRADEAPI CRspOptOrderField
{
  ...
  char          szOptTrdacct[18+1];	 //2015/8/3添加期权合约账户 
  char          szOptNum[16 + 1];           // 合约编码
  char          szOptCode[32 + 1];          // 合约代码
  char          szOptName[32 + 1];          // 合约简称
  char          szOptUndlCode[8 + 1];       // 标的证券代码
...
struct MATRADEAPI CReqOptCancelOrderField
{
  ...
  char          szOrderId[10 + 1];          // 合同序号
  int           iOrderBsn;                  // 委托批号
  char          szClientInfo[256 + 1];      // 客户端信息 包括硬盘号\CPU\机器名等
...
struct MATRADEAPI CRspOptCancelOrderField
{
  ...
  char          szTrdacct[10 + 1];          // 证券账户
  char          szSubacctCode[8 + 1];       // 证券账户子编码
  char			szOptTrdacct[18+1];	//2015/8/3 期权合约账户
...
struct MATRADEAPI CReqOptMaxTradeQtyField
{
  ...
  char          szStkbd[2 + 1];             // 交易板块 字典[STKBD]
  char          szTrdacct[10 + 1];          // 证券账户
  char          szOptNum[16 + 1];           // 合约编码 若为开平仓，合约编码不能为空
  char          szStkCode[8 + 1];           // 证券代码 若为证券锁定解锁，证券代码不能为空
  char          szOrderPrice[11 + 1];       // 委托价格 CPRICE4 开平仓时必送，行权与证券冻结解冻非必送
  int           iStkBiz;                    // 证券业务 字典[STK_BIZ]
  int           iStkBizAction;              // 证券业务行为 字典[STK_BIZ_ACTION]
  char          szCombNum[16+1];			// 组合编码	     2015/8/3
  char          szCombStraCode[16+1];		// 组合策略代码	 2015/8/3
  char          szLeg1Num[16+1];			// 成分一合约编码2015/8/3	
  char          szLeg2Num[16+1];			// 成分二合约编码2015/8/3
  char          szLeg3Num[16+1];			// 成分三合约编码2015/8/3	
  char          szLeg4Num[16+1];			// 成分四合约编码2015/8/3	
...
struct MATRADEAPI CRspOptMaxTradeQtyField
{
  ...
  char          szStkbd[2 + 1];             // 交易板块 字典[STKBD]
  char          szTrdacct[10 + 1];          // 证券账户
  char          szSubAcctcode[8+1];         // 证券账户子编码2015/8/3
  ...
  char          szLeg4Num[16+1];			// 成分四合约编码2015/8/3
  LONGLONG      llOrderQty;                 // 委托数量
  LONGLONG      llNoLmtOrderQty;            // 最大委托数量
};
...
struct MATRADEAPI CRspOptAcctField
{
  ...
  int           iOptMarginSn;               // 保证金比例模板编号  2015/8/5 删除
  char          szQuotaVal[21 + 1];         // 买入额度
  char          szQuotaValUsed[21+1];       // 占用买入额度			 2015/8/5
  char          chOpFlag;                   // 操作标志    			 2015/8/5
...
//行权指派欠券
 ...
 struct MATRADEAPI CRspOptRiskLvlField
 {
	...
	char     szRiskValue[8+1];			//风险率	RISK_VALUE	CRATE	9085
	char     szRiskValueRlt[8+1];		 //实时风险率	RISK_VALUE_RLT	CRATE	9086
	char     szStkexRiskValue[8+1];		 //风险率(交易所)	STKEX_RISK_VALUE	CRATE	9087
	char     szStkexRiskValueRlt[8+1];	 //实时风险率(交易所)	STKEX_RISK_VALUE_RLT	CRATE	9089
