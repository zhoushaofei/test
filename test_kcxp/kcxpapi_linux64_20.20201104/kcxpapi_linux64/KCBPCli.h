...
#define KCBP_XA_ENTRY_SIZE			32
#define KCBP_XA_PATH_SIZE			128
#define KCBP_XA_OPEN_SIZE			512
#define KCBP_XA_CLOSE_SIZE			128
#define KCBP_XA_OPTION_SIZE			512
#define KCBP_XA_SERIAL_SIZE			20
...
/* struct for kcbpxa's parameter */
typedef struct
{
	char szServerName[KCBP_SERVERNAME_MAX + 1]; //XA name
	int nProtocal;
	char szSwitchLoadFile[KCBP_XA_PATH_SIZE + 1];
	char szXAEntry[KCBP_XA_ENTRY_SIZE + 1];
	char szXAOpen[KCBP_XA_OPEN_SIZE + 1];
	char szXAClose[KCBP_XA_CLOSE_SIZE + 1];
	char szXASerial[KCBP_XA_SERIAL_SIZE + 1];
	char szXAOption[KCBP_XA_OPTION_SIZE];
} tagKCBPXAConnectOption;
...
KCBPCLI_API int KCBPCLISTDCALL KCBPCLI_CallEx(KCBPCLIHANDLE hHandle, char *ProgramName, int nFlag, void *pInput, int nInputLen, void **pOutput, int *pOutputLen);
...
KCBPCLI_API int KCBPCLISTDCALL KCBPCLI_GetCommArea(KCBPCLIHANDLE hHandle, void **p);
KCBPCLI_API int KCBPCLISTDCALL KCBPCLI_SetCommArea(KCBPCLIHANDLE hHandle, char *);
